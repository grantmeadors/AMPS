...
bandhilist = [2000 70.1 700.1 2000 2000 2000];
ylolist = 1e-6*1e23*[1.e-23 6.e-23 1.e-23 1.e-23 -1e6*1.e-24 -1e6*1.e-23];
yhilist = 1e23*[1.e-21 5.e-21 1.e-21 1.e-21 1.e-24 1.e-23];
bandnames = ['40_2000_Hz ';'40_70_Hz   ';'70_700_Hz  ';'200_2000_Hz';' 30_2000_Hz'; ' 20_2000_Hz'];
...
%  for iifo = 1:length(ifolist) 
  for iifo = 1:1
     ...
     fnameFull1 = strcat('~gmeadors/2012/12/13/AMPS/', fname1);
     data1 = load(fnameFull1);
     fname2 = sprintf('%s_%s_40_2000feedforward.txt',fnameroot,strtrim(windowlist(irun,:)));
     fnameFull2 = strcat('~gmeadors/2012/12/13/AMPS/', fname2);
     ...
     amppsd{irun,iifo} = resample((data1(:,3) - data2(:,3)),1,...
         resampleFactor, resampleFilterSize)./resample(data1(:,3),1,resampleFactor,resampleFilterSize);
     amppsdwt{irun,iifo} = resample((data1(:,5) - data2(:,5)), 1,...
         resampleFactor, resampleFilterSize)./resample(data1(:,5),1,resampleFactor,resampleFilterSize);
	...
	titlestr = sprintf('%s %s Average Ratio of Spectra, Before - After (%d-%d Hz)',run,ifo,round(bandlo),round(bandhi));
        title(titlestr)
