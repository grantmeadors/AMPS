...
def combSpectrum(targetDirectory, flag):
    ...
    histMarginBins = 5e-24 * np.arange(500., 1001.) 
    beforeMargin = np.sum(beforeArray, axis=1) 
    afterMargin = np.sum(afterArray, axis=1)
    histMarginBins450 = 1e-23 * np.arange(0., 401.) 
    beforeMargin450 = np.sum(beforeArray[:, 23:-1], axis=1)
    afterMargin450 = np.sum(afterArray[:, 23:-1], axis=1)
    plt.figure()
    p2 = plt.hist(beforeMargin, histMarginBins)
    plt.grid(True)
    plt.xlabel('Hoft')
    plt.ylabel('Histogram count')
    plt.title('Comb before feedforward, marginalized over all frequencies')
    plt.savefig(graphWholeRun + 'HistogramBeforeMargin.png')
    plt.savefig(graphWholeRun + 'HistogramBeforeMargin.pdf')
    plt.close()
    plt.figure()
    p3 = plt.hist(afterMargin, histMarginBins)
    plt.grid(True)
    plt.xlabel('Hoft')
    plt.ylabel('Histogram count')
    plt.title('Comb after feedforward, marginalized over all frequencies')
    plt.savefig(graphWholeRun + 'HistogramAfterMargin.png')
    plt.savefig(graphWholeRun + 'HistogramAfterMargin.pdf')
    plt.close()
    plt.figure()
    p4 = plt.hist(beforeMargin450)
    plt.grid(True)
    plt.xlabel('Hoft')
    plt.ylabel('Histogram count')
    plt.title('Comb before feedforward, marginalized over all frequencies >= 450 Hz')
    plt.savefig(graphWholeRun + 'HistogramBeforeMargin450.png')
    plt.savefig(graphWholeRun + 'HistogramBeforeMargin450.pdf')
    plt.close()
    plt.figure()
    p5 = plt.hist(afterMargin450, histMarginBins450)
    plt.grid(True)
    plt.xlabel('Hoft')
    plt.ylabel('Histogram count')
    plt.title('Comb after feedforward, marginalized over frequencies >= 450 Hz')
    plt.savefig(graphWholeRun + 'HistogramAfterMargin450.png')
    plt.savefig(graphWholeRun + 'HistogramAfterMargin450.pdf')
    plt.close()
