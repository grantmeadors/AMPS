...
bandlolist = [40   40 69.9 199.9 30 849.875];
bandhilist = [2000 70.1 700.1 2000 2000 850.125];
ylolist = [1.e-23 6.e-23 1.e-23 1.e-23 1.e-24 1e-23];
yhilist = [1.e-21 5.e-21 1.e-21 1.e-21 5.e-23 1e-22];
...
%  for iifo = 1:length(ifolist) 
  for iifo = 1:1
     ...
     fname1 = sprintf('%s_%s_40_2000beforeTukey.txt',fnameroot,strtrim(windowlist(irun,:)));
     fnameFull1 = strcat('~pulsar/feedforward/2012/11/05/AMPS/', fname1);
     data1 = load(fnameFull1);
     fname2 = sprintf('%s_%s_40_2000afterTukey.txt',fnameroot,strtrim(windowlist(irun,:)));
     fnameFull2 = strcat('~pulsar/feedforward/2012/11/05/AMPS/', fname2);
     ...
     freq{irun,iifo} = data1(:,1); %resample(data1(:,1), 1,...
         %resampleFactor, resampleFilterSize);
     amppsd{irun,iifo} = data1(:, 5); %resample(data1(:,5), 1,...
         %resampleFactor, resampleFilterSize);
     amppsdwt{irun,iifo} = data2(:, 5)%resample(data2(:,5), 1,...
         %resampleFactor, resampleFilterSize);
