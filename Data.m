classdef Data < handle
    ...
    methods
        function channels = Data(t0, t1, addenda)
            ...
            channels.durationTail =...
                ceil(channels.t(2)/addenda.s) * addenda.s - channels.t(2);
            ...
            if addenda.pipe == 1
                channelname =...
                    {strcat(channels.site, '1:LDAS-STRAIN')...
                     strcat(channels.site, '1:LSC-MICH_CTRL')...
                     strcat(channels.site, '1:LSC-PRC_CTRL')...
                     strcat(channels.site, '1:IFO-SV_STATE_VECTOR')...
                     strcat(channels.site, '1:LSC-DATA_QUALITY_VECTOR')};
            elseif addenda.pipe == 2
                channelname = ...
                    {strcat(channels.site, '1:LSC-DARM_ERR')...
                     strcat(channels.site, '1:LSC-MICH_CTRL')...
                     strcat(channels.site, '1:LSC-PRC_CTRL')...
                     strcat(channels.site, '1:IFO-SV_STATE_VECTOR')...
                     strcat(channels.site, '1:LSC-DATA_QUALITY_VECTOR')};
            ...
            if addenda.PRCfilter == 1
                ...
                if addenda.passFirstFlag == 1
                    if channels.duration > 512
                        ...
                        channels.noise = [addenda.passPRC;...
                            readFramesVerily(addenda.inputFileNOISE,...
                            channelname{3},channels.t(1)+512,...
                            channels.duration-512, 16384)];
                        disp('Check on PRC')
                    ...
                    channels.noise =...
                        readFramesVerily(addenda.inputFileNOISE,...
                        channelname{3},channels.t(1),...
                        channels.duration, 16384);
                end
            else
                ...
                if addenda.passFirstFlag == 1
                    if channels.duration > 512
                        if addenda.frameTailFlag == 0
                            ...
                            channels.darm = [addenda.passDARM;...
                                ...
                                channelname{1},channels.t(1)+512,...
                                channels.duration-512, 16384)];
                            disp('Check on DARM')
                        ...
                        elseif addenda.frameTailFlag == 1
                            channels.darm = [addenda.passDARM;...
                                readFramesVerily(addenda.inputFileDARM,...
                                channelname{1},channels.t(1)+512,...
                                (channels.tau2-channels.t(1)-512), 16384)];
                            addenda.destroyer('passDARM');
                        end
                    elseif addenda.frameTailFlag == 1;
                        if (channels.tau2-channels.t(2)) > 0
                            channels.darm = [addenda.passDARM;...
                                readFramesVerily(addenda.inputFileDARM,...
                                channelname{1},channels.t(2),...
                                (channels.tau2-channels.t(2)), 16384)];
                        else
                            % This situation is encountered at the edge
                            % case where a science segment ends on a frame.
                            channels.darm = addenda.passDARM;
                        end
                        addenda.destroyer('passDARM');
                    else
                    ...
                    channels.darm =...
                        readFramesVerily(addenda.inputFileDARM,...
                        channelname{1},channels.tau1,...
                        channels.durationPlus, 16384);
                end
                % Break the head of the segment off for later use,
                if addenda.frameHeadFlag == 1
                    channels.frameHead =...
                        channels.darm(...
                        1:...
                        (channels.Fs*( channels.t(1)- channels.tau1 )));
                end
                % Do the same with the tail
                channels.tailStartIndex =...
                    length(channels.darm) -...
                    channels.Fs*(channels.tau2 - channels.t(2)) + 1;
                if addenda.frameTailFlag == 1
                    channels.frameTail =...
                ...
                if addenda.passFirstFlag == 1
                    if channels.duration > 512
                        ...
                        channels.noise = [addenda.passMICH;...
                            readFramesVerily(addenda.inputFileNOISE,...
                            channelname{2},channels.t(1)+512,...
                            channels.duration-512, 16384)];
                        disp('Check on MICH')
                ...
                if addenda.pipe == 1
                    ...
                    channels.stateVector =...
                        readFramesVerily(addenda.inputFileDARM,...
                        channelname{4},channels.tau1,...
                        channels.durationPlus, 16);
                    channels.dqFlag =...
                        readFramesVerily(addenda.inputFileDARM,...
                        channelname{5},channels.tau1,...
                        channels.durationPlus, 1);
                end
            end
        ...
        function wienerFilter(channels)
            ...
            [zd, pd, kd] =...
                bilinear(zs', ps', 1/m, channels.Fs/resamplingFactor);
            ...
            bandEmphasize =...
                @(inputG, inputSOS, inputVector, inputResamplingFactor)...
            ...
            darm1 =...
                bandEmphasize(g1, sos1, channels.darm, resamplingFactor);
            noise1 =...
                bandEmphasize(g1, sos1, channels.noise, resamplingFactor);

            % Apply a strong low-pass
            [zh, ph, kh] =...
                butter(10, 70/(channels.Fs/(2*resamplingFactor)), 'high');
            ...
            function [output1, output2] = notcher(input1, input2, a, b)
                [z, p, k] =...
                    butter(10,...
                    [a b]./(channels.Fs/(2*resamplingFactor)), 'stop');
                [sos, g] = zp2sos(z, p, k);
                g = real(g);
            ...
            for ii = 1:length(notchSpots)
                [darm1, noise1] =...
                    notcher(darm1, noise1,...
                    notchSpots(ii) - 3, notchSpots(ii) + 3);
            end
        ...
        function feedforwardFilter(channels, za, pa, ka)
            ...
            channels.noise = filterZPKs(...
                za, pa, ka, channels.Fs, channels.noise);
        end
